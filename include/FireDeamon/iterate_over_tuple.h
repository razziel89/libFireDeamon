...
/**
 * \brief namespace containing templates that can be used to perform actions for every entry in a tuple.
 *
 * The "iteration" is no actual iteration as tuples are objects whose lengths have to be fully known
 * at compile time. Access functions also have ot be known at compile time. Hence, templating is used
 * to create a sequence 1..N where N is the length of the tuple over which to "iterate".
 */
namespace tuple_it
{
    ...
    //! \brief recursively generate a sequence of numbers and keep them in the template information
    ...
    /**
     * \brief Evaluate the functor for each element of the tuple. Not to be called directly.
     *
     * \param t pointer to T - the tuple over which to "iterate" (elements will be passed to the functor)
     * \param f F            - the functor who shall be called with \a t and \a r as arguments
     * \param seq<Is...>     - a struct that contains the sequence of numbers in its template information
     */
    ...
    /**
     * \brief Evaluate the functor for each element of the tuple. Can be called directly.
     *
     * This template also allows passing an additional argument to the functor.
     * \param t pointer to tuple - the tuple over which to "iterate" (elements will be passed to the functor)
     * \param r pointer to R     - an argument that will be passed to the functor
     * \param f F                - the functor who shall be called with \a t and \a r as arguments
     */
