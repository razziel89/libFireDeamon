...
/** \brief Compute the electrostatic potential due to molecular orbitals.
 *
 * Some matrices (P and S matrices) are usually computed on the level of
 * contracted Cartesian Gaussian functions. However, this functions needs them
 * <em>spread onto the primitives</em>, which means nothing more that, if a
 * contracted function has j primitives, the value has to be duplicated j times
 * in direct succession.
 */
void electrostatic_potential_orbitals(
        bool progress_reports,                  //!< whether or not to print progress reports during the computation
        int num_primitives,                     //!< int - the number of primitive functions making up the basis
        ...
        std::vector<double> potential_grid,     //!< std::vector<double> - a flat vector containing the Cartesian coordinates
                                                //! of the points at which to compute the potential
        std::vector<double>  P_matrix,          //!< std::vector<double> - a flat vector containing the first order density matrix.
                                                //! This matrix has to be <em>spread onto the primitives</em>
        std::vector<double> S_matrix,           //!< std::vector<double> - a flat vector containing the overlap matrix of the
        ...
        std::vector<double> *potential          //!< pointer to std::vector<double> - this vector will hold the computed potential
                                                //! in the same order as the coordinates were defined in \a potential_grid
        );
